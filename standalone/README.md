...
# 11) Send second touch for followup_due prospects
./geo outreach run \
  --campaign-file ./output/outreach/outreach-campaign.json \
  --provider dry-run \
  --run-followup-due

# 12) Manual status update (replied/won/lost)
./geo outreach update \
  --campaign-file ./output/outreach/outreach-campaign.json \
  --domain example.com \
  --new-status won \
...
# 13) Execute via webhook automation (n8n/Make/custom API)
./geo outreach run \
  --campaign-file ./output/outreach/outreach-campaign.json \
  --provider webhook \
...
# 14) Execute via built-in Apify provider (backlink-outreach-js adapter)
./geo outreach run \
  --campaign-file ./output/outreach/outreach-campaign.json \
  --provider apify \
...
# 15) Execute via custom command adapter
./geo outreach run \
  --campaign-file ./output/outreach/outreach-campaign.json \
  --provider command \
...
- `outreach-followup-sequences.md`, `outreach-followup.csv` generated by `outreach verify`
...
- second touch path: `outreach verify` marks due prospects and generates followup templates, then `outreach run --run-followup-due` sends them.
